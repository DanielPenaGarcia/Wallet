...
import { creditCardPaymentCycle } from '$lib/modules/cards/utils/credit-card-cycle';
import {
	nextInterestFreeInstallmentsAmount as cardNextInterestFreeInstallmentsAmount,
...
function creditCardReserve(
	...
	if (card.kind !== 'credit' || card.statementDay === null || card.paymentDueDay === null) return null;

	const cashExpenseAmount = card.cashExpenseAmount ?? 0;
	const nextInterestFreeInstallmentsAmount = cardNextInterestFreeInstallmentsAmount(card);
	const payableAmount = statementPayableAmount(card);
	const { dueDate } = creditCardPaymentCycle(card.statementDay, card.paymentDueDay, today);
	...
	const currentlyReservedAmount = reservedAmount(reserveMovements, {
		reserveKind: 'credit',
		targetId: card.id,
		cycleDueOn: nextDueDateIso,
		currencyCode: card.currencyCode
	});
	if (payableAmount <= 0 && currentlyReservedAmount <= 0) return null;

	const reserveAmount = Math.max(
		Math.ceil(payableAmount / semimonthsUntilDue) - currentlyReservedAmount,
		0
	);
	...
	return {
		...
		reservedAmount: currentlyReservedAmount,
		reservedAmountLabel: formatCurrencyFromMinorUnits(currentlyReservedAmount, card.currencyCode),
		currencyCode: card.currencyCode
	};
}
...
function creditBalanceTotal(cards: CardListItem[], currencyCode: string) {
	return cards
		.filter((card) => card.kind === 'credit' && card.currencyCode === currencyCode)
		.reduce((total, card) => total + card.currentBalance, 0);
}
...
export async function getDashboardSummary(referenceDate = new Date()): Promise<DashboardSummary> {
	...
	const creditDueDates = cards
		.filter(
			(card) => card.kind === 'credit' && card.statementDay !== null && card.paymentDueDay !== null
		)
		.map((card) =>
			creditCardPaymentCycle(card.statementDay ?? 1, card.paymentDueDay ?? 1, today).dueDate
		);
	...
	const reserves = expenses.map((expense) => {
		...
		const monthlyReservedAmount = reservedAmount(reserveMovements, {
			reserveKind: 'monthly',
			targetId: expense.id,
			cycleDueOn: nextDueDateIso,
		...
		const monthlyReserveAmount = Math.max(
			rawMonthlyReserveAmount - monthlyReservedAmount,
			0
		);
		const rawReserveAmount = semimonthlyReserveAmount(rawMonthlyReserveAmount, dueDate, today);
		const semimonthlyReservedAmount = reservedAmount(reserveMovements, {
		...
		const reserveAmount = Math.max(
			rawReserveAmount - semimonthlyReservedAmount,
			0
		);
		const status: 'paid' | 'reserved' | 'pending' = isCurrentCyclePaid(expense, dueDate)
			? 'paid'
			: monthlyReservedAmount > 0
				? 'reserved'
				: 'pending';
		...
		return {
			...
			reservedAmount: semimonthlyReservedAmount,
			reservedAmountLabel: formatCurrencyFromMinorUnits(
				semimonthlyReservedAmount,
			...
			monthlyReservedAmount,
			monthlyReservedAmountLabel: formatCurrencyFromMinorUnits(
				monthlyReservedAmount,
				expense.currencyCode
			),
			status,
			statusLabel: status === 'paid' ? 'Pagado' : status === 'reserved' ? 'Apartado' : 'Pendiente',
			currencyCode: expense.currencyCode
		};
	});
	const semimonthlyReserves = reserves.filter((reserve) => reserve.status !== 'paid');
	...
	const currentCreditBalance = creditBalanceTotal(cards, currencyCode);
	...
	return {
		...
		creditBalanceTotal: currentCreditBalance,
		creditBalanceTotalLabel: formatCurrencyFromMinorUnits(currentCreditBalance, currencyCode),
