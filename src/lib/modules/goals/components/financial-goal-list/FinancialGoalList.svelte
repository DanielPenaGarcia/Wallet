...
<section class="overflow-hidden rounded-lg border border-outline bg-surface shadow-sm">
	<div class="flex flex-col gap-4 border-b border-outline px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
		<div>
			<h2 class="text-lg font-bold text-on-surface">Objetivos financieros</h2>
			<p class="mt-1 text-sm text-on-surface-muted">Asigna una parte de tus ingresos a las cosas que quieres conseguir.</p>
		</div>
	...
	<div class="grid gap-3 border-b border-outline bg-primary/10 px-5 py-4 sm:grid-cols-2">
		<div>
			<p class="text-xs font-bold tracking-wide text-on-surface-muted uppercase">Ingresos asignados</p>
			<p class="mt-1 text-2xl font-bold text-primary">{allocatedPercentage}%</p>
		</div>
		<div>
			<p class="text-xs font-bold tracking-wide text-on-surface-muted uppercase">Disponible</p>
			<p class="mt-1 text-2xl font-bold text-on-surface">{100 - allocatedPercentage}%</p>
		</div>
	</div>
	...
	{#if goals.length === 0}
		<div class="px-6 py-12 text-center">
			...
			<p class="mt-3 font-bold text-on-surface-variant">Aún no hay objetivos</p>
			<p class="mt-1 text-sm text-on-surface-muted">Registra algo que quieras comprar o conseguir.</p>
		</div>
	{:else}
			...
			{#each goals as goal (goal.id)}
				<li class="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center">
					<div class="min-w-0 flex-1">
						...
						<p class="mt-1 text-sm text-on-surface-muted">Meta de {formatCurrencyFromMinorUnits(goal.targetAmount, goal.currencyCode)}</p>
					</div>
					<div class="sm:text-right">
						<p class="text-lg font-bold text-primary">{goal.allocationPercentage}%</p>
						<p class="text-xs font-semibold tracking-wide text-on-surface-muted uppercase">De cada ingreso</p>
					</div>
