...
<form method="POST" action="?/createExpense" class="grid gap-4 sm:grid-cols-2">
	<div class="grid gap-2 sm:col-span-2">
		...
		<Input id="expense-name" name="name" required maxlength={80} value={createFeedback?.values?.name ?? ''} placeholder="Ej. Renta" class="h-11 border-outline" aria-invalid={fieldError('name') ? 'true' : undefined} />
	...
	<div class="grid gap-2">
		...
		<Select.Root type="single" name="classification" required bind:value={classification} items={expenseClassificationOptions}>
			<Select.Trigger id="expense-classification" class="h-11 w-full border-outline px-3"><span>{selectedClassification}</span></Select.Trigger>
	...
	<div class="grid gap-2">
		...
		<Input id="expense-amount" name="amount" required type="number" min="0.01" step="0.01" value={createFeedback?.values?.amount ?? ''} placeholder="0.00" class="h-11 border-outline" aria-invalid={fieldError('amount') ? 'true' : undefined} />
	...
	<div class="grid gap-2">
		...
		<Input id="expense-currency" name="currencyCode" required maxlength={3} value={createFeedback?.values?.currencyCode ?? 'MXN'} class="h-11 border-outline uppercase" aria-invalid={fieldError('currencyCode') ? 'true' : undefined} />
	...
	<div class="grid gap-2">
		...
		<Input id="expense-statement-day" name="statementDay" type="number" min="1" max="31" step="1" value={createFeedback?.values?.statementDay ?? ''} placeholder="Ej. 14" class="h-11 border-outline" aria-invalid={fieldError('statementDay') ? 'true' : undefined} />
	...
	<div class="grid gap-2">
		...
		<Input id="expense-payment-due-day" name="paymentDueDay" type="number" min="1" max="31" step="1" value={createFeedback?.values?.paymentDueDay ?? ''} placeholder="Ej. 3" class="h-11 border-outline" aria-invalid={fieldError('paymentDueDay') ? 'true' : undefined} />
