	...
	<div class="flex min-h-12 items-center gap-3 border-b border-slate-100 px-3 py-2 last:border-b-0" style:padding-left={`${depth * 1.25 + 0.75}rem`}>
		{#if hasChildren}
			<button
				...
				aria-label={isExpanded ? `Ocultar subcategorías de ${category.name}` : `Mostrar subcategorías de ${category.name}`}
				disabled={searching}
	...
	{#if hasChildren && isExpanded}
		<ul>
			{#each category.children as child (child.id)}
				<CategoryTreeItem category={child} depth={depth + 1} {expandedIds} {searching} {onToggleExpanded} {onAddChild} {onEdit} {onDelete} />
			{/each}
		</ul>
	{/if}
</li>
