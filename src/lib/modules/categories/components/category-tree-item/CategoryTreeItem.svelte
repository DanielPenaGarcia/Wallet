...
<li>
  <div
    class="flex min-h-12 items-center gap-3 border-b border-slate-100 px-3 py-2 last:border-b-0"
    style:padding-left={`${depth * 1.25 + 0.75}rem`}
  >
    {#if hasChildren}
      <button
        type="button"
        class="grid size-8 shrink-0 place-items-center rounded-md text-slate-500 hover:bg-slate-100 hover:text-slate-900 focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:outline-none"
    ...
    {/if}
    {#if !isChild}<span
        class="size-3 shrink-0 rounded-full ring-2 ring-white shadow-sm"
    ...
    <div class="min-w-0 flex-1">
      <div class="flex flex-wrap items-center gap-2">
        ...
        {#if category.isEssential}<span
            class="rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-bold text-emerald-700"
            >Esencial</span
          >{/if}
      </div>
      {#if category.children.length > 0}<p class="text-xs text-slate-400">
