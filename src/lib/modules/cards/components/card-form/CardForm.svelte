...
<form method="POST" action={mode === 'create' ? '?/createCard' : '?/updateCard'} class="grid gap-4 sm:grid-cols-2">
	...
	<div class="grid gap-2">
		...
		{#if mode === 'create'}
			<Select.Root type="single" name="kind" required bind:value={kind} items={cardTypes}>
				<Select.Trigger id={`${idPrefix}-kind`} class="h-11 w-full border-outline px-3"><span>{kind === 'credit' ? 'Crédito' : 'Débito'}</span></Select.Trigger>
				<Select.Content>
		...
		{:else}
			...
			<Input id={`${idPrefix}-kind`} value={kind === 'credit' ? 'Crédito' : 'Débito'} disabled class="h-11 border-outline" />
		{/if}
	</div>

	<div class="grid gap-2">
		<Label for={`${idPrefix}-alias`}>Alias</Label>
		<Input id={`${idPrefix}-alias`} name="alias" required maxlength={60} value={matchingFeedback?.values?.alias ?? card?.alias ?? ''} placeholder="Ej. Compras del hogar" class="h-11 border-outline" aria-invalid={fieldError('alias') ? 'true' : undefined} />
	...
	<div class="grid gap-2">
		...
		<Select.Root type="single" name="bankId" required disabled={banks.length === 0} bind:value={bankId} items={banks.map((bank) => ({ value: bank.id, label: bank.name }))}>
			<Select.Trigger id={`${idPrefix}-bank`} class="h-11 w-full border-outline px-3" aria-invalid={fieldError('bankId') ? 'true' : undefined}><span class="truncate">{selectedBankName}</span></Select.Trigger>
			<Select.Content>
	...
	<div class="grid gap-2">
		...
		<Input id={`${idPrefix}-last-digits`} name="lastFourDigits" required inputmode="numeric" minlength={4} maxlength={4} value={matchingFeedback?.values?.lastFourDigits ?? card?.lastFourDigits ?? ''} placeholder="1234" oninput={keepOnlyFourDigits} class="h-11 border-outline" aria-invalid={fieldError('lastFourDigits') ? 'true' : undefined} />
		{#if fieldError('lastFourDigits')}<span class="text-xs text-destructive">{fieldError('lastFourDigits')}</span>{/if}
	</div>

	<div class="grid gap-2">
		<Label for={`${idPrefix}-currency`}>Moneda</Label>
		<Input id={`${idPrefix}-currency`} name="currencyCode" required maxlength={3} value={matchingFeedback?.values?.currencyCode ?? card?.currencyCode ?? 'MXN'} class="h-11 border-outline uppercase" aria-invalid={fieldError('currencyCode') ? 'true' : undefined} />
	...
	<div class="grid gap-2">
		...
		<Input id={`${idPrefix}-initial-balance`} name="initialBalance" required type="number" min="0" step="0.01" value={matchingFeedback?.values?.initialBalance ?? (card ? (card.initialBalance / 100).toFixed(2) : '0.00')} class="h-11 border-outline" aria-invalid={fieldError('initialBalance') ? 'true' : undefined} />
	...
	{#if kind === 'credit'}
		<div class="grid gap-2">
			...
			<Input id={`${idPrefix}-credit-limit`} name="maximumOfferedCredit" required type="number" min="0" step="0.01" value={matchingFeedback?.values?.maximumOfferedCredit ?? (card ? ((card.maximumOfferedCredit ?? 0) / 100).toFixed(2) : '')} placeholder="0.00" class="h-11 border-outline" aria-invalid={fieldError('maximumOfferedCredit') ? 'true' : undefined} />
			{#if fieldError('maximumOfferedCredit')}<span class="text-xs text-destructive">{fieldError('maximumOfferedCredit')}</span>{/if}
		</div>
		<div class="grid gap-2">
			<Label for={`${idPrefix}-statement-day`}>Día de corte</Label>
			<Input id={`${idPrefix}-statement-day`} name="statementDay" required type="number" min="1" max="31" value={matchingFeedback?.values?.statementDay ?? card?.statementDay ?? ''} class="h-11 border-outline" aria-invalid={fieldError('statementDay') ? 'true' : undefined} />
		...
		<div class="grid gap-2">
			...
			<Input id={`${idPrefix}-payment-day`} name="paymentDueDay" required type="number" min="1" max="31" value={matchingFeedback?.values?.paymentDueDay ?? card?.paymentDueDay ?? ''} class="h-11 border-outline" aria-invalid={fieldError('paymentDueDay') ? 'true' : undefined} />
	...
	{:else}
		<div class="grid gap-2">
			...
			<Input id={`${idPrefix}-account`} name="accountId" required maxlength={80} value={matchingFeedback?.values?.accountId ?? card?.accountId ?? ''} placeholder="Ej. Cuenta principal" class="h-11 border-outline" aria-invalid={fieldError('accountId') ? 'true' : undefined} />
			{#if fieldError('accountId')}<span class="text-xs text-destructive">{fieldError('accountId')}</span>{/if}
		</div>
	{/if}
