...
<div class="grid gap-6">
	...
	<section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
		<div class="rounded-lg border border-outline bg-surface p-4 shadow-sm sm:p-5">
			<div class="flex items-start justify-between gap-3">
				<div class="min-w-0">
					...
					<p class="mt-2 break-words text-xl font-bold text-on-surface sm:text-2xl">{money(summary.current.availableMoneyCents)}</p>
				</div>
				<LandmarkIcon class="size-5 shrink-0 text-primary" />
			</div>
		...
		<div class="rounded-lg border border-outline bg-surface p-4 shadow-sm sm:p-5">
			<div class="flex items-start justify-between gap-3">
				<div class="min-w-0">
					...
					<p class="mt-2 break-words text-xl font-bold text-on-surface sm:text-2xl">{money(summary.current.consumedCreditCents)}</p>
				</div>
		...
		<div class="rounded-lg border border-outline bg-surface p-4 shadow-sm sm:p-5">
			...
			<p class="mt-2 break-words text-xl font-bold text-on-surface sm:text-2xl">{money(summary.monthlyActivity.incomeCents)}</p>
		...
		<div class="rounded-lg border border-outline bg-surface p-4 shadow-sm sm:p-5">
			...
			<p class="mt-2 break-words text-xl font-bold text-on-surface sm:text-2xl">{money(summary.monthlyActivity.paidExpenseCents + summary.monthlyActivity.creditPurchaseCents)}</p>
	...
	<section class="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
		<div class="rounded-lg border border-outline bg-surface shadow-sm">
			<div class="flex flex-col items-start gap-3 border-b border-outline px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
				<div class="flex items-center gap-2">
					<RepeatIcon class="size-5 shrink-0 text-primary" />
			...
			<div class="grid gap-4 p-4 lg:grid-cols-2 sm:p-5">
				<div class="rounded-md bg-surface-subtle p-4">
					...
					{#if summary.upcomingIncome}
						<p class="mt-2 break-words font-bold text-on-surface">{summary.upcomingIncome.title}</p>
						<p class="mt-1 text-sm text-on-surface-muted">{formatIsoDate(summary.upcomingIncome.date)}</p>
						<p class="mt-3 break-words text-xl font-bold text-on-surface">{money(summary.upcomingIncome.expectedAmountCents)}</p>
					{:else}
				...
				<div class="grid gap-3">
					{#if summary.upcomingExpenses.length > 0}
						{#each summary.upcomingExpenses as expense (expense.id)}
							<div class="grid gap-2 rounded-md bg-surface-subtle p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
								<div class="min-w-0">
									<div class="flex items-center gap-2">
										<span class="size-2.5 shrink-0 rounded-full" style:background-color={expense.categoryColor}></span>
										<p class="min-w-0 break-words text-sm font-bold text-on-surface sm:truncate">{expense.name}</p>
									</div>
									<p class="mt-1 break-words text-xs text-on-surface-muted sm:truncate">{expense.categoryName} · {formatIsoDate(expense.date)}</p>
								</div>
								<p class="text-sm font-bold text-on-surface sm:text-right">{money(expense.amountCents)}</p>
							</div>
						{/each}
					{:else}
		...
		<div class="rounded-lg border border-outline bg-surface shadow-sm">
			<div class="flex flex-col items-start gap-3 border-b border-outline px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
				<div class="flex items-center gap-2">
					<ListIcon class="size-5 shrink-0 text-primary" />
				...
				{#if summary.recentMovements.length > 0}
					{#each summary.recentMovements as movement (movement.id)}
						<div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
							<div class="min-w-0">
								<p class="break-words text-sm font-bold text-on-surface sm:truncate">{movement.title}</p>
								<p class="mt-1 break-words text-xs text-on-surface-muted">
									{movementLabels[movement.type]} · {formatIsoDate(movement.occurredAt)}{movement.accountLabel ? ` · ${movement.accountLabel}` : ''}
								</p>
							</div>
							<p class="text-sm font-bold sm:text-right {movementTone(movement.type)}">{money(movement.amountCents, movement.currencyCode)}</p>
						</div>
					{/each}
				{:else}
	...
	<section class="grid gap-6 xl:grid-cols-2">
		<div class="rounded-lg border border-outline bg-surface shadow-sm">
			<div class="flex flex-col items-start gap-3 border-b border-outline px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
				<div class="flex items-center gap-2">
					<HandCoinsIcon class="size-5 shrink-0 text-primary" />
			...
			<div class="grid gap-4 p-4 sm:grid-cols-2 sm:p-5">
				<div class="rounded-md bg-surface-subtle p-4">
					...
					<p class="mt-2 break-words text-xl font-bold text-on-surface">{money(summary.loans.borrowedOutstandingCents)}</p>
					{#if summary.loans.nextBorrowedInstallment}
						<p class="mt-2 break-words text-xs text-on-surface-muted">
							{summary.loans.nextBorrowedInstallment.name}<br />
							{formatIsoDate(summary.loans.nextBorrowedInstallment.dueDate)} · {money(summary.loans.nextBorrowedInstallment.amountCents, summary.loans.nextBorrowedInstallment.currencyCode)}
						</p>
					{:else}
				...
				<div class="rounded-md bg-surface-subtle p-4">
					...
					<p class="mt-2 break-words text-xl font-bold text-on-surface">{money(summary.loans.lentOutstandingCents)}</p>
					{#if summary.loans.nextLentInstallment}
						<p class="mt-2 break-words text-xs text-on-surface-muted">
							{summary.loans.nextLentInstallment.name}<br />
							{formatIsoDate(summary.loans.nextLentInstallment.dueDate)} · {money(summary.loans.nextLentInstallment.amountCents, summary.loans.nextLentInstallment.currencyCode)}
						</p>
					{:else}
		...
		<div class="rounded-lg border border-outline bg-surface shadow-sm">
			<div class="flex flex-col items-start gap-3 border-b border-outline px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
				<div class="flex items-center gap-2">
					<WalletCardsIcon class="size-5 shrink-0 text-primary" />
				...
				{#if summary.creditCards.length > 0}
					{#each summary.creditCards as card (card.accountId)}
						<a href={`/cuentas/${card.accountId}`} class="block rounded-md bg-surface-subtle p-4 transition hover:bg-surface-hover focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none">
							<div class="flex items-start justify-between gap-3">
								<div class="min-w-0">
									<p class="break-words font-bold text-on-surface sm:truncate">{card.name}</p>
									<p class="mt-1 break-words text-xs text-on-surface-muted sm:truncate">{card.bankName}{card.cardLastFourDigits ? ` · **** ${card.cardLastFourDigits}` : ''}</p>
								</div>
								<span class="shrink-0 rounded-full px-2 py-1 text-xs font-bold {card.isActive ? 'bg-primary-container text-primary' : 'bg-surface-pressed text-on-surface-muted'}">{card.isActive ? 'Activa' : 'Inactiva'}</span>
							</div>
							<div class="mt-4 grid gap-3 sm:grid-cols-2">
								<p class="grid gap-0.5 text-sm text-on-surface-muted">
									<span>Consumido</span>
									<span class="break-words font-bold text-on-surface">{money(card.consumedBalanceCents)}</span>
								</p>
								<p class="grid gap-0.5 text-sm text-on-surface-muted">
									<span>Disponible</span>
									<span class="break-words font-bold text-on-surface">{money(card.availableCreditCents)}</span>
								</p>
								<p class="grid gap-0.5 text-sm text-on-surface-muted">
									<span>Exigible</span>
									<span class="break-words font-bold text-on-surface">{money(card.statementOutstandingCents)}</span>
								</p>
								<p class="grid gap-0.5 text-sm text-on-surface-muted">
									<span>Vence</span>
									<span class="font-bold text-on-surface">{formatIsoDate(card.paymentDueDate)}</span>
								</p>
							</div>
						</a>
					{/each}
				...
				{#if summary.goals.length > 0}
					{#each summary.goals as goal (goal.id)}
						<div>
							<div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
								<div class="min-w-0">
									<p class="break-words font-bold text-on-surface sm:truncate">{goal.name}</p>
									<p class="mt-1 text-xs text-on-surface-muted">Restante: {money(goal.remainingAmountCents)}</p>
								</div>
								<p class="text-sm font-bold text-on-surface sm:text-right">{goal.progressPercentage}%</p>
							</div>
