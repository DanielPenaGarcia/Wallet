...
<section class="grid gap-5">
	<div class="flex flex-col gap-3 rounded-lg border border-outline bg-surface p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
		<div>
			<h2 class="text-lg font-bold text-on-surface">Préstamos</h2>
			<p class="mt-1 text-sm text-on-surface-muted">Separa deuda por pagar y dinero por cobrar sin mezclarlo con ingresos o gastos.</p>
		</div>
		<ActionButton type="button" onclick={() => (createOpen = true)}><PlusIcon />Nuevo préstamo</ActionButton>
	</div>
