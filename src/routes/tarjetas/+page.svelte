...
<svelte:head><title>Tarjetas | Mi Cartera</title></svelte:head>
...
<div class="space-y-6">
	...
	<Dialog.Root bind:open={createOpen}>
		<Dialog.Content class="sm:max-w-2xl">
			<Dialog.Header>
				<Dialog.Title>Nueva tarjeta</Dialog.Title>
				<Dialog.Description>Agrega los datos iniciales para comenzar a administrarla.</Dialog.Description>
			</Dialog.Header>
			{#if data.banks.length === 0}
				<p class="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">Primero registra un banco desde Configuración.</p>
			{/if}
	...
	<Dialog.Root open={viewingCard !== null} onOpenChange={(open) => { if (!open) viewingCard = null; }}>
		<Dialog.Content class="sm:max-w-3xl">
			<Dialog.Header>
				<Dialog.Title>Detalles de {viewingCard?.alias ?? 'tarjeta'}</Dialog.Title>
	...
	<Dialog.Root open={editingCard !== null} onOpenChange={(open) => { if (!open) editingCard = null; }}>
		<Dialog.Content class="sm:max-w-2xl">
			<Dialog.Header>
				<Dialog.Title>Editar tarjeta</Dialog.Title>
	...
	<Dialog.Root open={deletingCard !== null} onOpenChange={(open) => { if (!open) deletingCard = null; }}>
		<Dialog.Content>
			<Dialog.Header>
				<Dialog.Title>Eliminar {deletingCard?.alias ?? 'tarjeta'}</Dialog.Title>
				<Dialog.Description>Confirma que deseas retirar esta tarjeta de la lista activa.</Dialog.Description>
			</Dialog.Header>
