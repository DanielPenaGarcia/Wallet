...
export const actions = {
	createCard: async ({ request }) => {
		...
		try {
			...
			return { action: 'create-card' as const, success: 'Tarjeta registrada correctamente.' };
		} catch (error) {
	...
	updateCard: async ({ request }) => {
		...
		try {
			...
			return { action: 'update-card' as const, success: 'Tarjeta actualizada correctamente.' };
		} catch (error) {
			if (
	...
	deleteCard: async ({ request }) => {
		...
		if (!result.success) {
			return fail(400, { action: 'delete-card' as const, message: 'Tarjeta inválida.' });
		}

		try {
			await deleteCard(result.data.id);
			return { action: 'delete-card' as const, success: 'Tarjeta eliminada correctamente.' };
		} catch (error) {
